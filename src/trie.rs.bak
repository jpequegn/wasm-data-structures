...
impl Trie {
    ...
    // Static helper function that doesn't borrow self
    fn delete_recursive_helper(
        node: &mut TrieNode,
        word: &str,
        depth: usize,
        node_count: &mut u32,
        ...
        if let Some(child) = node.children.get_mut(&ch) {
            let result = Self::delete_recursive_helper(child, word, depth + 1, node_count);
    ...
    // Internal helper for autocomplete that returns Vec<String>
    fn autocomplete_internal(&self, prefix: &str) -> Vec<String> {
        let mut current = &self.root;
...
impl Trie {
    ...
    pub fn delete(&mut self, word: &str) -> bool {
        let result =
            Self::delete_recursive_helper(&mut self.root, word, 0, &mut self.metrics.node_count);
        if result.is_some() {
            self.size -= 1;
            self.update_metrics();
            true
...
mod tests {
    ...
    fn test_autocomplete_no_matches() {
        ...
        let completions = trie.autocomplete_internal("world");
